...
function initAudioPlayer(){
  ...
  audio.addEventListener('timeupdate',apUpdateProgress);
  audio.addEventListener('loadedmetadata',apUpdateProgress);
  audio.addEventListener('ended',()=>{
    const playIcon=document.getElementById('ap-play-icon');
    if(playIcon){playIcon.dataset.lucide='play';lucide.createIcons();}
  });
  audio.addEventListener('error',()=>{
    const bar=document.getElementById('audio-player-bar');
    if(bar)bar.innerHTML='<span style="font-size:12px;color:var(--g4);padding:0 16px">Audio playback error.</span>';
  });
}

function apTogglePlay(){
  const audio=document.getElementById('session-audio');
  const icon=document.getElementById('ap-play-icon');
  if(!audio)return;
  if(audio.paused){
    audio.play().catch(()=>toast('Could not play audio'));
    if(icon){icon.dataset.lucide='pause';lucide.createIcons();}
  } else {
    audio.pause();
    if(icon){icon.dataset.lucide='play';lucide.createIcons();}
  }
}
...
function apToggleMute(){
  const audio=document.getElementById('session-audio');
  const icon=document.getElementById('ap-vol-icon');
  if(!audio)return;
  audio.muted=!audio.muted;
  if(icon){
    icon.dataset.lucide=audio.muted?'volume-x':'volume-2';
    lucide.createIcons();
  }
}
...
function apUpdateProgress(){
  ...
  if(timeEl){
    ...
    const dur=audio.duration&&isFinite(audio.duration)?_fmtTime(audio.duration):'--:--';
...
async function saveNotes(){
  ...
  try{
    await fetch(`/api/session/${SESSION_ID}/save_notes`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({notes:text})});
    toast('Notes saved');
