...
let openTabs = [];
let activeTab = 'learn';
let currentView = 'pdf';   // 'pdf' | 'text'
let textFormatted = false; // reformatText only runs once
...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  // If no PDF URL, start in text view
  if(!SESSION_PDF_URL){
    switchView('text');
  } else {
    // Show PDF by default; hide toggle text btn if no raw text
    if(!HAS_RAW_TEXT){
      const vtt=document.getElementById('vt-text');
      if(vtt)vtt.style.display='none';
    }
  }
...
function switchView(mode){
  ...
  if(mode==='pdf'){
    ...
    if(vtPdf)  {vtPdf.classList.add('active');}
    if(vtText) {vtText.classList.remove('active');}
    hideSelToolbar();
  } else {
    ...
    if(vtPdf)  {vtPdf.classList.remove('active');}
    if(vtText) {vtText.classList.add('active');}
    attachSelectionListener();
  }
}

// ── Text reformatter (identical to text_session) ───────────────────────────
function reformatText(){
  const el=document.getElementById('reader-body');
  if(!el)return;
  let raw=el.textContent||el.innerText;
  if(!raw.trim()){el.innerHTML='<p style="color:var(--g4)">No text content extracted.</p>';return;}
  raw=raw
    .replace(/\r\n/g,'\n').replace(/\t/g,'  ')
    .replace(/[ ]{2,}/g,' ').replace(/\n{3,}/g,'\n\n')
    .replace(/ *\n */g,'\n').trim();
  ...
  for(const block of blocks){
    ...
    if(/^```/.test(first)||lines.every(l=>/^( {4}|\t)/.test(l))){
      ...
      html+=lines.map(l=>{const c=l.indexOf(':');const k=esc(l.slice(0,c).trim());const v=inlineFmt(l.slice(c+1).trim());return `<p><strong>${k}:</strong> ${v}</p>`;}).join('');
...
function loadAnnotations(){
  try{const saved=sessionStorage.getItem('ann_'+SESSION_ID);if(saved)annotations=JSON.parse(saved);}catch(e){}
  renderAnnotations();
}
function saveAnnotationsState(){sessionStorage.setItem('ann_'+SESSION_ID,JSON.stringify(annotations));}
function addAnnotation(text,color,markId){
  annotations.push({id:markId,text,color});saveAnnotationsState();renderAnnotations();
  if(firstHighlight){firstHighlight=false;openAnnotationsAuto();}
}
function removeAnnotation(id){
  annotations=annotations.filter(a=>a.id!==id);saveAnnotationsState();
...
function scrollToMark(id){
  const mark=document.querySelector(`mark[data-ann-id="${id}"]`);
  if(!mark){toast('Highlight not found');return;}
  // Switch to text view first if in PDF mode
  if(currentView==='pdf')switchView('text');
  ...
  mark.classList.add('ann-pulse');setTimeout(()=>mark.classList.remove('ann-pulse'),1800);
}
...
// ── SELECTION TOOLBAR (text view only) ────────────────────────────────────
let _savedRange=null,_savedText='';
...
async function selExplain(){
  const text=_savedText||getSelectedText();if(!text)return;
  hideSelToolbar();
  const overlay=document.getElementById('explain-overlay');
  const out=document.getElementById('explain-text');
  out.textContent='Explaining…';overlay.style.transform='translateY(0)';
  try{
    const res=await fetch(`/api/session/${SESSION_ID}/text_explain`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text})});
...
function selChat(){const text=_savedText||getSelectedText();if(!text)return;hideSelToolbar();toast('Chat from selection coming soon');}
...
function selHighlight(color='yellow'){
  ...
  if(currentView!=='text'){toast('Switch to Text view to highlight');return;}
  hideSelToolbar();
  const markId=Date.now();
  try{
    const mark=document.createElement('mark');
    mark.dataset.color=color;mark.dataset.annId=markId;mark.textContent=text;
    _savedRange.deleteContents();_savedRange.insertNode(mark);
  ...
  try{
    ...
    if(data.ok&&data.chapters?.length){
      ...
      document.getElementById('chapters-output').innerHTML=data.chapters.map((c,i)=>`
        <div class="chapter-card" onclick="scrollToChapter(${JSON.stringify(c.start_snippet||'')})">
...
function scrollToChapter(snippet){
  if(!snippet)return;
  if(currentView==='pdf'){switchView('text');}
  const body=document.getElementById('reader-body');
...
function toggleListen(){
  ...
  // Switch to text view to get content
  if(currentView==='pdf')switchView('text');
...
// ── FIGURES (Mistral OCR extracted images) ───────────────────────────────
let _figuresLoaded = false;

async function checkFigures() {
  // Peek to see if there are any figures — show tool button only if yes
  try {
    const r = await fetch(`/api/session/${SESSION_ID}/ocr_figures`);
    const d = await r.json();
    if (d.figures && d.figures.length > 0) {
      const btn = document.getElementById('figures-tool-btn');
    ...
    grid.innerHTML = figs.map((fig, i) => `
      <div class="figure-card">
        <img class="figure-img"
             src="data:${fig.mime};base64,${fig.b64}"
             alt="Figure ${i+1} (Page ${fig.page})"
             loading="lazy"
